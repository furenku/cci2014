...
function calendarioHTML() {
  ...
  $echo .= '<div id="dropdownsFechas" class="row">'.$dropdowns.'</div>';
  $echo .= '<div id="diasMes" class="row">&nbsp</div>';
  $echo .= '</div>';
...
function seleccionar_anho( $anho ) {
  ...
  foreach( $meses as $mes => $diasMes ) {
    $output[] = $nombresMeses[(int)$mes-1];
  }  
  
  $dd = generarDropdown( "mes", $output[0], $output );

  die( $dd );
  
}

function seleccionar_mes( $anho, $mes ) {
  global $eventos;
  global $datosMeses;
  $mesIndex = array_search($mes,array_keys($datosMeses));
  
  $dias = $eventos[$anho][$mesIndex + 1];

  $numDiasMes = $datosMeses[$mes];
  ...
  for( $i = 0; $i < $numDiasMes; $i++) {
    ...
    $ul = "";
    $eventosDia = "";
    if( in_array( $numDia, array_keys($dias) ) ) {
      $class = " diaConEventos";

      $ul = '<ul class="eventosDia">';
      foreach($dias[$numDia] as $evento){
        $ul .= '<li class="evento">'.$evento['ttl'].'</li>';
      }
      $ul .= '</ul>';
    ...
    $diasLis .= '<li class="diaCalHorizontal'.$class.'">'.$numDia.$ul.'</li>';
  }
  die( $diasLis );
}
